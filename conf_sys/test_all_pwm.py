...
def test_pwm_pin_mapping():
    ...
    for i, (chip, channel) in enumerate(pwm_configs):
        ...
        try:
            ...
            pwm.frequency = 1000  # 1kHz - good for LED testing
            pwm.duty_cycle = 0.5  # 50% duty cycle - clearly visible
            ...
            print("   - LED at medium brightness (50% duty cycle)")
            print("   - AC voltage ~1.65V on multimeter")  
            print("   - 1kHz square wave on oscilloscope")
            
            # Run for 10 seconds
            for second in range(10, 0, -1):
                print(f"\r   ⏳ {second} seconds remaining...", end="", flush=True)
                time.sleep(1)
...
def quick_test_specific_pins():
    ...
    for chip, channel, expected_pin in likely_mappings:
        try:
            ...
            # Breathing effect to make it very obvious
            print(f"   🔄 Running breathing LED effect on {expected_pin}...")
            
            import math
            for i in range(60):  # 3 seconds of breathing
                brightness = (math.sin(i * 0.1) + 1) / 2  # 0 to 1
