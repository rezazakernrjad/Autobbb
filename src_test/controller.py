#!usr/bin/env python3
"""
Controller module for turning left or right also move forward and reverse
Uses pin_lib and pwm_lib to control GPIO pins and PWM signals.
pin8_12 - left wheel control
pwm8_13 - left pwm control
pin8_17 - right wheel control
pwm8_19 - right pwm control
pwm9_14 - illumination effects
turning left includs reversing right wheel and moving left wheel forward
    set PIN8_12 to HIGH: PIN::set_left_control("reverse")
    set PIN8_13 to 0% PWM: PWMController::set_left_duty(0)
    set PIN8_17 to LOW: PIN::set_right_control("forward")
    set PIN8_19 to 100% PWM: PWMController::set_right_duty(100)
turning right includs reversing left wheel and moving right wheel forward
forward moves both wheels forward
reverse moves both wheels in reverse.
"""
import time
from pin_lib import PIN
from pwm_lib import PWMController
pin = PIN()
pwm = PWMController()
class WheelController:
    def __init__(self):
        print("To Controll Wheels")
    def tuen_left(self, angle):
        pin.set_left_control("reverse")
        pwm.set_left_duty(0)
        pin.set_right_control("forward")
        pwm.set_right_duty(100)
        print(f"turn left for {angle} degree")
        time.sleep(angle)  # Simulate time taken to turn
    def turn_right(self, angle):
        pin.set_left_control("forward")
        pwm.set_left_duty(100)
        pin.set_right_control("reverse")
        pwm.set_right_duty(0)
        time.sleep(angle)  # Simulate time taken to turn
        print(f"turn right for {angle} degree")
    def forward(self, speed):
        pin.set_left_control("forward")
        pin.set_right_control("forward")
        pwm.set_all_wheels_duty(speed)
        print(f"move forward at speed {speed}")
    def reverse(self):
        print("move reverse")
        pwm.set_all_wheels_duty(0)
        pin.set_left_control("reverse")
        pin.set_right_control("reverse")
    def brake(self):
        print("brake all movement")
        pwm.set_all_wheels_duty(0)
        pin.set_left_control("forward")
        pin.set_right_control("forward")