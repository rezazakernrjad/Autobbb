...
class _BBBControllerState extends State<BBBController> {
  ...
  void dispose() {
    ...
    if (connectedDevice != null) {
      connectedDevice!.disconnect().catchError((e) {
        debugPrint('⚠️ Error disconnecting during dispose: $e');
      });
    }
    super.dispose();
  }
  ...
  Future<void> _initBluetoothIOS() async {
    debugPrint('📱 iOS/iPadOS Bluetooth initialization');
    try {
      await Future.delayed(const Duration(seconds: 2));
    } catch (e) {
      debugPrint('❌ iOS Bluetooth init error: $e');
    }
  }
        ...
        for (ScanResult result in results) {
          if (result.device.name.isNotEmpty && 
            ...
            if (!devicesList.any((d) => d.id == result.device.id)) {
              debugPrint('✅ Found Auto BBB device: ${result.device.name}');
              setState(() {
  ...
  Future<void> connectToDevice(BluetoothDevice device) async {
    debugPrint('🔵 Connect requested for: ${device.name}');
    
    // Prevent multiple simultaneous connections
    if (isConnecting) {
      debugPrint('⚠️ Already connecting, please wait...');
      return;
    }
    ...
    try {
      ...
      if (currentState == BluetoothConnectionState.connected) {
        debugPrint('⚠️ Device already connected, disconnecting first...');
        await device.disconnect();
        await Future.delayed(const Duration(milliseconds: 500));
      }
      
      debugPrint('📡 Initiating connection...');
      ...
      var verifyState = await device.connectionState.first.timeout(const Duration(seconds: 2));
      debugPrint('📊 Connection state verified: $verifyState');
      ...
      _connectionStateSubscription = device.connectionState.listen((state) {
        ...
        debugPrint('🔗 [$timestamp] Connection state changed: $state');
        if (state == BluetoothConnectionState.disconnected) {
          debugPrint('⚠️ [$timestamp] Device disconnected!');
          // Only handle unexpected disconnects, not intentional ones
          if (mounted && !_isDisconnecting) {
            debugPrint('⚠️ [$timestamp] Unexpected disconnect detected');
            _resetState();
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
            ...
            debugPrint('ℹ️ [$timestamp] Intentional disconnect, ignoring');
          }
        }
      });
  ...
  Future<void> _discoverServicesWithRetry(BluetoothDevice device) async {
    ...
    for (int i = 0; i < retries; i++) {
      try {
        debugPrint('🔎 Discovering services (attempt ${i + 1}/$retries)...');
          ...
          if (service.uuid.toString().toUpperCase() == SERVICE_UUID.toUpperCase()) {
            ...
            for (BluetoothCharacteristic characteristic in service.characteristics) {
              ...
              if (charUuid == RX_UUID.toUpperCase()) {
                ...
                rxFound = true;
                debugPrint('✅ Found RX characteristic (write to BBB)');
              ...
              if (charUuid == TX_UUID.toUpperCase()) {
                ...
                txFound = true;
                debugPrint('✅ Found TX characteristic (receive from BBB)');
                ...
                try {
                  ...
                  debugPrint('📡 Enabled notifications on TX characteristic');
                  ...
                  txSubscription = txCharacteristic!.lastValueStream.listen((value) {
                    if (value.isNotEmpty) {
                      ...
                      debugPrint('📥 Response from BBB: "$response"');
                    ...
                    debugPrint('❌ TX subscription error: $error');
                  });
                  sendCommand("illumination 2"); // Turn on lamps on connect
                  debugPrint('✅ Subscribed to TX notifications');
                } catch (e) {
                  debugPrint('⚠️ Failed to subscribe to TX notifications: $e');
                }
              }
            }
        ...
        if (!serviceFound) {
          ...
          debugPrint('⚠️ TX characteristic not found - no responses from BBB');
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
        ...
        debugPrint('❌ Service discovery error (attempt ${i + 1}): $e');
        if (i < retries - 1) {
          debugPrint('🔄 Retrying service discovery...');
  ...
  Future<void> sendCommand(String command) async {
    if (!isConnected || rxCharacteristic == null) {
      debugPrint('❌ Cannot send: Not connected or no RX characteristic');
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      ...
      if (rxCharacteristic!.properties.writeWithoutResponse) {
        try {
          debugPrint('📝 Writing WITHOUT response (iOS/BBB compatible mode)...');
          ...
          if (bytes.length <= maxChunkSize) {
            ...
            debugPrint('✅ Sent ${bytes.length} bytes');
          } else {
            // Send in chunks
            debugPrint('📦 Splitting into chunks...');
            for (int i = 0; i < bytes.length; i += maxChunkSize) {
              ...
              debugPrint('✅ Sent chunk ${(i / maxChunkSize).floor() + 1}: ${chunk.length} bytes');
        ...
        debugPrint('❌ WriteWithoutResponse not supported by characteristic');
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _disconnect() async {
    ...
    if (!isConnected && connectedDevice == null) {
      debugPrint('⚠️ Already disconnected');
      _resetState();
    ...
    if (connStateSub != null) {
      debugPrint('🚫 Cancelling connection state subscription first...');
      try {
        await connStateSub.cancel();
        _connectionStateSubscription = null;
      } catch (e) {
        debugPrint('⚠️ Error cancelling connection subscription: $e');
      }
    }
    ...
    try {
      ...
      if (txCharacteristic != null && deviceToDisconnect != null) {
        try {
          debugPrint('🔕 Disabling TX notifications...');
          await txCharacteristic!.setNotifyValue(false).timeout(
            const Duration(seconds: 1),
          );
          debugPrint('✅ Notifications disabled');
        } on TimeoutException catch (_) {
          debugPrint('⚠️ Disable notifications timeout');
        } catch (e) {
      ...
      if (txSub != null) {
        debugPrint('🚫 Cancelling TX subscription...');
        try {
          await txSub.cancel();
        } catch (e) {
          debugPrint('⚠️ Error cancelling TX subscription: $e');
        }
      }
      ...
      if (deviceToDisconnect != null) {
        ...
        try {
          ...
          debugPrint('⚠️ Disconnect error (continuing cleanup): $e');
        }
      }
      
      debugPrint('✅ Disconnect sequence completed');
        ...
        child: Column(
          children: [
            ...
            if (isConnected) ...[
              ...
              //     debugPrint('Ended change on $pwmValue');
              //   },
              // ),
              ...
              Row(
                ...
                children: [
                  Column(
                    ...
                    children: [
                      Text('Acceleration: ${pwmValue.round()}%', 
                        style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                      ...
                      VerticalSlider(
                        ...
                        onChangeEnd: (value){
                          double zeroValue = 50;
                          setState(() => pwmValue = zeroValue);
                          value = 0;
                          sendCommand("forward ${value.round()}");
                          debugPrint('Stop on $value');
                        },
                    ...
                    children: [
                      ...
                      VerticalSlider(
                        ...
                        onChanged: (value) {
                          setState(() => turnValue = value);
                          if (value < 50) {
                            sendCommand("turn_left ${(50 - value.round()) * 2}");
                          } else if (value > 50) {
                            sendCommand("turn_right ${(value.round() - 50) * 2}");
                          } else {
                            sendCommand("turn_dummy");
                          }
                        },
                        onChangeEnd: (value){
                          ...
                          setState(() => turnValue = zeroValue);
                          sendCommand("turn_end");
                          debugPrint('Ended change on $turnValue');
                        },
                        activeColor: Colors.blue,
                      ),
                      const SizedBox(height: 10),
      ...
      child: RotatedBox(
        ...
        child: SliderTheme(
          data: SliderTheme.of(context).copyWith(
            ...
            trackShape: _CustomTrackShape(thickness: thickness),
            thumbShape: RoundSliderThumbShape(
              enabledThumbRadius: thickness / 2 + 5,
            ),
            overlayShape: RoundSliderOverlayShape(
              overlayRadius: thickness / 2 + 10,
            ),
...
// Custom track shape for better control
class _CustomTrackShape extends RoundedRectSliderTrackShape {
  final double thickness;
  
  _CustomTrackShape({required this.thickness});
  ...
  Rect getPreferredRect({
    ...
    final double trackHeight = thickness;
